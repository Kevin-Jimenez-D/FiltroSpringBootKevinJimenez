...
public interface CostumerService {
    ...
    ResponseEntity<Costumer> getCostumerById(String id);

    // Método para guardar un nuevo cliente
    ResponseEntity<Costumer> saveCostumer(Costumer costumer);

    // Método para actualizar un cliente existente
    ResponseEntity<Costumer> updateCostumer(Costumer costumer);

    // Método para eliminar un cliente por su identificador
    ResponseEntity<Void> deleteCostumerById(String id);
